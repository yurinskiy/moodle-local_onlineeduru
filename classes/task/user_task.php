...
SQL;
        ...
        while (($jobs = $DB->get_records_sql($sql, null, 0, 10)) && $i <= self::LIMIT_PACKAGE) {
...
SQL;
        ...
        mtrace('Отправка оценок пользователей с курса - завершена');
    }
    ...
    public function sendProgress()
    {
        ...
        mtrace("Отправка прогресса пользователей с курса");

        $sql = <<<SQL
select j.*
  from {local_onlineeduru_progress} j
 where j.gis_courseid is not null
   and j.gis_userid is not null
   and j.sessionid is not null
   and j.uuid_request is null
...
SQL;
        ...
        while (($jobs = $DB->get_records_sql($sql, null, 0, 10)) && $i <= self::LIMIT_PACKAGE) {
            ...
            try {
                ...
                foreach ($jobs as $job) {
                    ...
                    $job->uuid_request = $uuid;
                    $DB->update_record('local_onlineeduru_progress', $job);

                    $data[] = $item;
                }

                mtrace(json_encode($data, JSON_NUMERIC_CHECK));

                $api = new api();
                $response = $api->sendProgress($uuid, json_encode($data, JSON_NUMERIC_CHECK));
                mtrace($response);
                ...
                if ($api->getStatus() != 200) {
                    ...
                    foreach ($responseJson as $item) {
                        ...
                        $job = $DB->get_record('local_onlineeduru_progress', [
                            'gis_courseid' => $item->course_id,
                            'gis_userid' => $item->user_id,
                            'sessionid' => $item->session_id,
                        ]);
        ...
        mtrace('Отправка прогресса пользователей с курса - завершена');
    }
}